	...
	<div class="card card-outline card-primary">
          ...
          <div class="col-md-12">
            ...
            <div class="row">
              <div class="col-sm-6 form-group ">
                ...
                <select name="livreur_id" id="" class="form-control input-sm select2">
                  <option value=""></option>
                  <?php
                    $livreures = $conn->query("SELECT *,concat(street,', ',city,', ',state,', ',zip_code,', ',country) as address FROM livreures");
                    while($row = $livreures->fetch_assoc()):
                  ?>
                  <option value="<?php echo $row['id'] ?>" <?php echo isset($livreur_id) && $livreur_id == $row['id'] ? "selected":'' ?>><?php echo $row['livreur_code']. ' | '.(ucwords($row['address'])) ?></option>
                <?php endwhile; ?>
                </select>
              </div>
            </div>
  	...
  	<div class="card-footer border-top border-info">
  		<div class="d-flex w-100 justify-content-center align-items-center">
  			<button class="btn btn-flat  bg-gradient-primary mx-2" form="manage-client">Enrejistrer</button>
  			<a class="btn btn-flat bg-gradient-secondary mx-2" href="./index.php?page=client_list">Annuler</a>
  		</div>
  	</div>
	</div>
</div>
