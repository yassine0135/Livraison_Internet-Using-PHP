...
$result = $conn->query("SELECT * FROM colis where id = ".$_GET['id']);
$qry = $result->fetch_array();
foreach($qry as $k => $v){
    $$k = $v;
}
if($to_livreur_id > 0 || $from_livreur_id > 0){
	$to_livreur_id = $to_livreur_id  > 0 ? $to_livreur_id  : '-1';
	$from_livreur_id = $from_livreur_id  > 0 ? $from_livreur_id  : '-1';
	$livreur_result = $conn->query("SELECT *, concat(street, ', ', city, ', ', state, ', ', zip_code, ', ', country) as address FROM livreur where id in ($to_livreur_id, $from_livreur_id)");
	$livreur = [];
	while ($row = $livreur_result->fetch_assoc()) {
		...
		<div class="row">
			<div class="col-md-6">
				<div class="callout callout-info">
					...
					<dd><?php echo ucwords($livreur[$from_livreur_id]) ?></dd>
				</div>
				<div class="callout callout-info">
			...
			<div class="col-md-6">
				<div class="callout callout-info">
					<b class="border-bottom border-primary">colis Details</b>
						<div class="row">
							<div class="col-sm-6">
								<dl>
									<dt>Wight:</dt>
...
<script>
	$('#update_status').click(function(){
		uni_modal("Update Status of: <?php echo $reference_number ?>","manage_colis_status.php?id=<?php echo $id ?>&cs=<?php echo $status ?>","")
	})
</script>