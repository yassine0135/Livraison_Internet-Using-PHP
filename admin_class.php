...
Class Action {
	...
	function save_livreur(){
		...
		if(empty($id)){
			...
			while($i == 0){
				...
				$chk = $this->db->query("SELECT * FROM livreur where livreur_code = '$bcode'")->num_rows;
				if($chk <= 0){
					$i = 1;
				}
			}
			$data .= ", livreur_code='$bcode' ";
			$save = $this->db->query("INSERT INTO livreur set $data");
		}else{
			$save = $this->db->query("UPDATE livreur set $data where id = $id");
		}
		if($save){
			return 1;
		}
	}
	function delete_livreur(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM livreur where id = $id");
		if($delete){
			return 1;
		}
	}
		...
		foreach($price as $k => $v){
			...
			if(empty($id)){
				...
				while($i == 0){
					...
					$chk = $this->db->query("SELECT * FROM colis where reference_number = '$ref'")->num_rows;
					if($chk <= 0){
						$i = 1;
					}
				}
				$data .= ", reference_number='$ref' ";
				if($save[] = $this->db->query("INSERT INTO colis set $data"))
					$ids[]= $this->db->insert_id;
			}else{
				if($save[] = $this->db->query("UPDATE colis set $data where id = $id"))
					$ids[] = $id;
			}
		}
	...
	function delete_colis(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM colis where id = $id");
		if($delete){
			return 1;
		}
	}
	function update_colis(){
		extract($_POST);
		$update = $this->db->query("UPDATE colis set status= $status where id = $id");
		$save = $this->db->query("INSERT INTO colis_tracks set status= $status , colis_id = $id");
		if($update && $save)
			return 1;  
	}
	...
	function get_report(){
		extract($_POST);
		$data = array();
		$get = $this->db->query("SELECT * FROM colis where date(date_created) BETWEEN '$date_from' and '$date_to' ".($status != 'all' ? " and status = $status " : "")." order by unix_timestamp(date_created) asc");
		$status_arr = array("Article accepte","Expédié","En-Transit","Delivered");
